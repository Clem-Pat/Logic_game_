...
class Window(tk.Tk):
    def __init__(self, cmd_caller, name='Puzzle'):
        ...
        self.t0 = time.time()
        self.x, self.y = 0, 0
        self.length, self.height = self.winfo_screenwidth(), self.winfo_screenheight()
        self.config()
    ...
    def config(self):
        ...
        self.cmd_buttons = [Tkinter_command_button(self, i) for i in range(3)]
    ...
    def reload_matrix(self):
        ...
        for i in range(matrix_size[0]):
            for j in range(matrix_size[1]):
                self.buttons_in_grid[i][j]['text'] = str(self.matrix_caller[i][j])
                if self.buttons_in_grid[i][j]['text'] == '0':
                    self.buttons_in_grid[i][j].config(text='', bg='light blue', cursor='arrow')
                else:
                    self.buttons_in_grid[i][j].config(bg='blue', cursor='hand2', command= self.buttons_in_grid[i][j].move_case)
        self.update()